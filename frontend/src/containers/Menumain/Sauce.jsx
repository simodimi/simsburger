...
const Sauce = () => {
  ...
  useEffect(() => {
    const loadSnack = async () => {
      try {
        ...
        const boissonsAvecStatut = sauce.map((p) => {
        ...
        setSauceData(boissonsAvecStatut);
      } catch (error) {
  ...
  if (loading) return <div>Chargement des sauces...</div>;
  return (
    <div className="">
      <ScrollPage
        title="Nos sauces"

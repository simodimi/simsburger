...
const Wraps = () => {
  ...
  useEffect(() => {
    const loadSnack = async () => {
      try {
        ...
        const boissonsAvecStatut = wrap.map((p) => {
        ...
        setWrapData(boissonsAvecStatut);
      } catch (error) {
  ...
  if (loading) return <div>Chargement des Wraps...</div>;
  return (
    <div className="">
      <ScrollPage
        title="Nos Wraps"
